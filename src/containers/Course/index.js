...
const Course = ({ course, inProcess, match, getCourse, notSubscribed, history, user, getCert }) => {

  useEffect(() => {
    // ComponentDidMount
    const { params } = match;
    if (!course || (course && course.id !== params.id))
      getCourse(params.id);
  }, []);

  useEffect(() => {
    const { params } = match;
    const user_subscriptions = user && user.subscription_ids ? user.subscription_ids : []
    const isSubscribedCategory = user_subscriptions.includes(parseInt(params.id, 10))
  ...
  return (
    <StaticPage pageClass='profile' headerData={headerData.autorized}>
      <section className="sec-category-content" style={{ marginBottom: '100px' }}>
        <div className="container">
          <div className='course-progress-wrapper'>
            <div className='course-progress-item'>
              <span className='course-progress-header'>Progress of passage lesson</span>
              <p className='course-progress-info'>{course.progress || 0}%</p>
            </div>
            <div className='course-progress-item'>
              <span className='course-progress-header'>Overall performance</span>
              <p className='course-progress-info'>{`${course.cpd_points_count || 0}/${course.tutorials_count * 10 || 0}`}</p>
            </div>
            <div className='course-progress-item'>
              <span className='course-progress-header'>Passed lessons</span>
              <p className='course-progress-info'>{`${course.completed_count || 0}/${course.tutorials_count || 0}`}</p>
            </div>
            <div className='course-progress-item' onClick={() => getCertHandler(course.id)}>
              <span className='course-progress-header'>Certificate: <strong>{course.completed_count != course.tutorials_count ? 'Unavaialble' : 'Available'}</strong></span>
          ...
          <div className='course-progress-lessons'>
            <div className='course-progress-lessons-table-header'>
              <span className='course-progress-lessons-table-title'>Title of the Module</span>
              <span className='course-progress-lessons-table-result'>Results</span>
            </div>
              ...
              course.tutorials.map((tutorial, index) => (
                ...
                >
                  <div className='course-progress-lessons-title-wrapper'>
                    <span className='course-progress-lessons-header'>{`Lesson ${index + 1}`}</span>
                    <span className='course-progress-lessons-title'>{tutorial.title}</span>
                  </div>
...
const mapStateToProps = ({ auth: { user }, course: { course, notSubscribed, inProcess } }) => ({
  user,
  course,
  notSubscribed,
});
