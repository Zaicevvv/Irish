...
const EditCourseForm = ({ onFormSubmit, handlerDelete, reqErrors, category, handlerLessonDelete, inProcess }) => {
    ...
    const targetImage = category && category.icon ? category.icon.url : placeholder;
        ...
        <div className='editCourseForm'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} >
                <div className='actionButtons'>
                    <a className='alertColor' onClick={handleClickOpen}>DELETE COURSE</a>
                    {!inProcess ? (
                        <button disabled={inProcess} type="submit">SAVE CHANGES</button>
                    ) : (
                ...
                <div className='w-660'>
                    <div className='mb-30 row'>
                        <div className='placeholder_img'>
                            <label htmlFor='course_img'>
                                {targetImage && !deleteFlag ? (
                                    <img src={config.REACT_APP_ROOT + targetImage} className='w-full' />
                                ) : (
                        ...
                        <div>
                            <input onChange={fileChangeHandler} type='file' name='course_img' id='course_img' className='avatar_input' />
                            {/* <p className='upoload_descr'>Add image</p> */}
                            <label htmlFor='course_img' className='ava_change_label'>Change</label>
                            <button className='ava_delete_label' onClick={imageDeleteHandler}>Delete</button>
                        </div>
                    </div>
                    ...
                    />
                    {errors && errors.title && <span className='error'>Please, add valid title</span>}
                    ...
                    />
                    {errors && errors.description && <span className='error'>Please, add valid description</span>}

                    {reqErrors && reqErrors.title && reqErrors.title.length > 0 && reqErrors.title.map((error) => <span className='error'>{error}<br /></span>)}

                    <p className='input_group_label'>List of the Module:</p>

                    {/* LIST OF LESSONS */}
                    {category.tutorials.map((item, index) => {
                        return <LessonListItem category_id={category.id} handlerDelete={handlerLessonDelete} key={index} item={item} index={index} />
                    })}

                    <Link to={routeToLessonEdit(category.id)} className="button courseFormSubmit fill">Add Lessons</Link>
                </div>
            </form>
            <Dialog
                open={open}
