        ...
        <div className='createCourseForm'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} >
                <div className='actionButtons'>
                    <Link to={ROUTE_TO_ADMIN_COURSES} className='alertColor'>Cancel</Link>
                ...
                <div className='w-660'>
                    <div className='mb-30 row'>
                        ...
                        <div>
                            ...
                            <label htmlFor='course_img' className='ava_change_label'>Add image</label>
                            <button onClick={imageDeleteHandler} className='ava_delete_label' >Delete</button>
                        </div>
                    </div>
                    <input
                    ...
                    {reqErrors.title && reqErrors.title.length > 0 && reqErrors.title.map((error) => <span className='error'>{error}<br /></span>)}

                    {/* <p className='formTip'>Must be minimum 6 characters long, with at least one capital letter and one number.</p> */}

                    <button type="submit" className="button courseFormSubmit fill">Add Lessons</button>
                </div>
            </form>
        </div>
    )
