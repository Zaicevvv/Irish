...
class EditLesson extends Component {
    ...
    if (id && id != "new") {
      this.props
        ...
        .then((res) => {
          if (
            res.value.data &&
            _.get(res, "action.payload.data.data.tutorial")
          ) {
            ...
            const data = {
              ...
              tutorial_videos_attributes:
                responseData.tutorial_videos_attributes.length == 0
                  ? [{ title: "", description: "", video_url: "" }]
                  : responseData.tutorial_videos_attributes,
              spd_quizzes_attributes:
                responseData.spd_quizzes_attributes.length == 0
                  ? [
                      {
                        question: "",
                        correct: true,
  ...
  handleSubmit = async () => {
    ...
    let filteredVideos = lesson.tutorial_videos_attributes.filter(
    ...
    _.mapKeys(data, (value, key) => {
      if (key === "spd_quizzes_attributes") {
        value.forEach((el) => {
          ...
          //   formData.append(
          //     "spd_quizzes_attributes[]" + keyQuestion,
          //     valueQuestion
          //   );
          // });

          formData.append("spd_quizzes_attributes[]correct", el.correct);
          formData.append("spd_quizzes_attributes[]id", el.id);
          formData.append("spd_quizzes_attributes[]question", el.question);
          formData.append(
            "spd_quizzes_attributes[]tutorial_id",
            el.tutorial_id
          );
          if (el.image === "delete") {
            formData.append("spd_quizzes_attributes[]image", null);
          }
          if (el.image && !el.image.url && el.image !== "delete") {
            formData.append("spd_quizzes_attributes[]image", el.image);
          }
        });
      ...
      <StaticPage pageClass="edit_lesson" headerData={headerData.admin}>
        <div className="container">
          <div className="lesson_action_buttons row justify-between align-center">
            ...
            <div>
              <Link to={routeToCourseEdit(course_id)} className="alertColor">
                Cancel
              </Link>
              {!inProcess ? (
                <button disabled={inProcess} onClick={this.handleSubmit}>
                  {id && id !== "new" ? "Save" : "Add"} lesson
                </button>
              ) : (
          ...
          <div className="edit_lesson_name">
            <p className="edit_lesson_headline">
              {id && id !== "new" ? "Edit" : "Adding"} lesson
            </p>
          ...
          <div className="row justify-center align-center tabs_btns">
            <button
              type="button"
              className="tab_switcher active"
              onClick={(e) => {
                this.tabSwitch(e.target, "topic");
              }}
            >
              {" "}
              Topic
          ...
          <div className="tab active" id="topic">
            <InputWithEditor
              ...
              value={lesson.topic}
              handleChange={this.handleChangeTopic}
            />
            <div className="attachemnts_wrapper">
                    ...
                    return (
                      <div className="row align-center" key={index}>
                        <a
                          ...
                          href={config.REACT_APP_ROOT + item.document.url}
                          target="_blank"
                        >
                          Document {index + 1}
                        </a>{" "}
                        <span
                          ...
                          onClick={() =>
                            this.deleteAttachmentHandler(item.id, index)
                          }
                        >
                          X
                        </span>
              ...
              {lesson.attachments_files &&
              lesson.attachments_files.length < 3 ? (
                <div>
                  <input
                    onChange={(e) =>
                      this.changeAttachmentHandler(e.target.files)
                    }
                    id="attachment_add"
          ...
          <div className="tab" id="video">
            ...
            {lesson.tutorial_videos_attributes &&
              lesson.tutorial_videos_attributes.map((item, index) => {
                return (
                  <div className="video_form_wrapper" key={index}>
                    {" "}
                    <VideoForm
                      video={item}
                      index={index}
            ...
            <button
              onClick={() => this.pushVideo()}
              type="button"
              className="add_video_btn row align-center"
            >
              {" "}
              <span>+</span> Add one more video
            </button>
          </div>

          <div className="tab" id="quiz">
            <p className="tab_header">Add Quiz</p>
            {lesson.spd_quizzes_attributes &&
              lesson.spd_quizzes_attributes.map((item, index) => {
                return (
                  <div className="quiz_form_wrapper" key={index}>
                    {" "}
                    <QuizForm
                      item={item}
                      index={index}
                      key={index}
                      handleChange={this.handleQuizchange}
                    />{" "}
                  </div>
                );
              })}
